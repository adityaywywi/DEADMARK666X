local EggContents = {
    ["PetEgg"] = {"Bunny", "Dog", "Golden Lab"},
    ["UncommonEgg"] = {"Black Bunny", "Cat", "Chicken", "Deer"},
    ["RareEgg"] = {"Orange Tabby", "Spotted Deer", "Pig", "Rooster", "Monkey"},
    ["LegendaryEgg"] = {"Cow", "Silver Monkey", "Sea Otter", "Polar Bear", "Turtle"},
...
local function createESP(egg)
    ...
    local nameClean = egg.Name:gsub("%s+", ""):lower()

    for known, pets in pairs(EggContents) do
        if nameClean == known:lower() and not egg:FindFirstChild("ESPLabel") then
